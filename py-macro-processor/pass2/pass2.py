...
"""
mdtab = []
mdt_filepath: str = input( "Enter MDT filepath: " )
with open( mdt_filepath , "r" ) as file:
    mdt_contents: str = file.read()
...
for line_tokens in mdt_line_tokens:
    ...
    if len( line_tokens ) == 3:
        ...
        if entry.operand1[3].isdigit():
            entry.operand1_index = int( entry.operand1[3] )
        entry.operand2 = line_tokens[2]
        if entry.operand2[3].isdigit():
            entry.operand2_index = int( entry.operand2[3] )
    mdtab.append( entry )
...
def process_call_statement( call_statement: str ):
...
while True:
    process_call_statement( input( "Enter call statement: " ) )